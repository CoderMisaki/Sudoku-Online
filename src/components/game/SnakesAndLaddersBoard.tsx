...
export const SnakesAndLaddersBoard: React.FC<SnakesAndLaddersBoardProps> = ({ broadcastSnakesState }) => {
  ...
  const activePlayerIds = useMemo(() => {
    const ids = Object.values(players)
      ...
      .map((p) => p.id);
    if (ids.length === 0 && userId) return [userId];
    return ids;
  }, [players, userId]);
  ...
  useEffect(() => {
    if (!snakesState || !snakesState.ladders || snakesState.ladders.length === 0) {
      ...
      if (broadcastSnakesState) {
        broadcastSnakesState(initial);
      }
    }
  }, [snakesState, room?.difficulty, activePlayerIds, updateSnakesState, broadcastSnakesState]);
  ...
  const frozenTurns = useMemo(() => snakesState?.frozenTurns || {}, [snakesState?.frozenTurns]);
    ...
    activePlayerIds.forEach((pId) => {
      res[pId] = visualPositions[pId] ?? serverPositions[pId] ?? 1;
    });
    return res;
  }, [activePlayerIds, visualPositions, serverPositions]);

  const myFrozenCount = userId ? frozenTurns[userId] || 0 : 0;

  const currentTurn = snakesState?.currentTurnUserId || activePlayerIds[0];
  const isMyTurn = currentTurn === userId || activePlayerIds.length === 1;
    ...
    <div className="flex flex-col items-center gap-4 w-full max-w-2xl mx-auto select-none">
      ...
      <div className="relative w-full aspect-square border-2 border-border bg-card rounded-2xl shadow-xl p-1 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none z-30">
          {Object.entries(effectivePositions).map(([pId, pos]) => {
            const p = players[pId];
            const pName = p?.username || (pId === userId ? 'Kamu' : 'Player');
            ...
            return (
              ...
              >
                <div className="mb-0.5 px-1 py-0.2 text-[9px] font-bold bg-background/90 text-foreground border border-border rounded-md shadow-sm whitespace-nowrap">
                  {pName}
                </div>
                <div
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-bold text-sm">
              {players[currentTurn || '']?.username || (currentTurn === userId ? 'Kamu' : 'Menunggu...')} {isMyTurn ? '(Giliran Kamu)' : ''}
            </span>
          </div>
        </div>
