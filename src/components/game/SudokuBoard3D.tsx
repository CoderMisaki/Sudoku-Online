...
export const SudokuBoard3D: React.FC<SudokuBoard3DProps> = ({
  ...
  const updateTileTextureRef = useRef(updateTileTexture);
  useEffect(() => {
    updateTileTextureRef.current = updateTileTexture;
  }, [updateTileTexture]);
  ...
  useEffect(() => {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        tiles.push(tileMeta);
        updateTileTextureRef.current(tileMeta, true);
      }
    }
    ...
    const animate = (currentTime: number) => {
      ...
      tiles.forEach(tile => {
        const isSelected = currentSelected?.row === tile.row && currentSelected?.col === tile.col;
        ...
        // Kedap-kedip untuk jawaban salah di mode collaborative — hilang saat dikoreksi benar
        const cell = gridRef.current?.[tile.row]?.[tile.col];
        const shouldBlink = Boolean(cell?.isWrong && roomRef.current?.mode === 'collaborative' && cell?.value !== null);
        const topMat = tile.materials[2] as THREE.MeshStandardMaterial;
        if (shouldBlink) {
          const blinkPhase = (Math.sin(currentTime * 0.0074) + 1) / 2; // 0..1, periode ~850ms
          const opacity = 0.25 + 0.75 * blinkPhase; // 0.25 .. 1, sinkron dengan CSS animate-collab-wrong
          if (!topMat.transparent) topMat.transparent = true;
          topMat.opacity = opacity;
        } else {
          if (topMat.opacity !== 1 || topMat.transparent) {
            topMat.opacity = 1;
